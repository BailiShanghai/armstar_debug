...
#define _IPSS_OTP_USER_NON_SECURE_SIZE (8)
#define _IPSS_OTP_USER_SECURE_SIZE (8)
...
static hal_ret_t _check_dubhe_otp_data_blank(void)
{
    ...
    PAL_LOG_INFO("Check OTP data in dubhe...\n");
    ...
    if (_GET_IPSS_LCS_TYPE(dubhe_otp_data.lcs) != IPSS_LCS_CM_DATA) {
        PAL_LOG_ERR("Bad dubhe otp life cycle: 0x%x\n",
...
#if 1	////Pochin debug
  hal_ret_t _update_dubhe_otp_data(void)
#else
...
#if 1	////Pochin Modify ==> Only if all content of otp are zero, copy otp data from flash
