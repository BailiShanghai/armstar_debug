 ...
 *  http://csrc.nist.gov/publications/fips/fips196/fips-197.pdf
 */
...
/* magic string: "AES" */
#define MBEDTLS_AES_MAGIC (0x414553U)

/**
 * Note: enum value CE_ENCRYPT on ce_lite_cipher.h is 0.
 *       but original MBEDTLS_AES_ENCRYPT is 1.
 */
#define MBEDTLS_AES_GET_DRV_MODE(_md_) \
        (((_md_) == MBEDTLS_AES_ENCRYPT) ? CE_ENCRYPT : CE_DECRYPT)
#define MBEDTLS_AES_GET_DRV_KEYSEL(_tp_) \
        (((_tp_) == MBEDTLS_AES_KL_KEY_MODEL) ? CE_KLAD_KEY_MODEL : CE_KLAD_KEY_ROOT)

/**
 * key type
 */
typedef enum mbedtls_aes_key_type_t {
    MBEDTLS_AES_USER_KEY = 0,         /**< user key */
    MBEDTLS_AES_SEC_KEY,              /**< secure key */
} mbedtls_aes_key_type;

/**
 * private ce_aes_data context.
 */
typedef struct mbedtls_ce_aes_data {
    union {
        struct {
            uint8_t key[CE_MAX_AES_KEY];
            uint32_t keybits;
        } user;
        ce_sec_key_t sec;
    };
    mbedtls_aes_key_type key_type;
    struct ce_cipher_ctx cipher;
} mbedtls_ce_aes_data_t;
 ...
 *| ------------------------------------ | -------------------------- |
 *| MBEDTLS_ERR_AES_ALLOC_FAILED         | CE_ERROR_OOM               |
 *| MBEDTLS_ERR_AES_INVALID_KEY_LENGTH   | CE_ERROR_BAD_KEY_LENGTH    |
 *| MBEDTLS_ERR_AES_INVALID_INPUT_LENGTH | CE_ERROR_BAD_INPUT_LENGTH  |
 *| MBEDTLS_ERR_AES_BAD_INPUT_DATA       | CE_ERROR_BAD_PARAMS        |
 *| MBEDTLS_ERR_AES_BAD_INPUT_DATA       | CE_ERROR_BAD_FORMAT        |
 *| MBEDTLS_ERR_AES_BAD_INPUT_DATA       | CE_ERROR_BAD_PADDING       |
 *| MBEDTLS_ERR_AES_HW_ACCEL_FAILED      | Other CE Error Code        |
 *| 0                                    | CE_SUCCESS                 |
 *| ------------------------------------ | -------------------------- |
 */
static int _convert_retval_to_mbedtls(int errno)
{
    ...
    default:
        errno = MBEDTLS_ERR_AES_HW_ACCEL_FAILED;
        break;
    }
    return errno;
}
...
{
    sec_key->sel = MBEDTLS_AES_GET_DRV_KEYSEL(key->sel);
...
static int _aes_set_key(mbedtls_aes_context *ctx)
{
    int ret = CE_SUCCESS;

    if (MBEDTLS_AES_USER_KEY == ctx->acd->key_type) {
        ret = ce_cipher_setkey(&ctx->acd->cipher,
                               ctx->acd->user.key,
                               ctx->acd->user.keybits);
    } else if (MBEDTLS_AES_SEC_KEY == ctx->acd->key_type) {
...
void mbedtls_aes_init(mbedtls_aes_context *ctx)
{
    ...
    if ((MBEDTLS_AES_MAGIC == ctx->magic) &&
        (ctx->acd != NULL)) {
        PAL_LOG_ERR("#WARN %s %d ctx may double init\n",
                     __func__, __LINE__);
    }
    pal_memset(ctx, 0x00, sizeof(*ctx));
    ctx->acd = (mbedtls_ce_aes_data_t *)pal_calloc(1,
                        sizeof(*(ctx->acd)));
                           ...
                           unsigned int keybits)
{
    ...
    ctx->acd->key_type = MBEDTLS_AES_USER_KEY;
finish:
    return ret;
}
                              ...
                              mbedtls_aes_sec_key_t *key)
{
    ...
    /* 1. Check mbedtls aes key type. */
    if ((MBEDTLS_AES_KL_KEY_MODEL != key->sel) &&
        (MBEDTLS_AES_KL_KEY_ROOT != key->sel)) {
        ret = MBEDTLS_ERR_AES_BAD_INPUT_DATA;
        goto finish;
    }
    if ((128 != key->ek1bits) && (256 != key->ek1bits)) {
        ret = MBEDTLS_ERR_AES_INVALID_KEY_LENGTH;
        goto finish;
    }
    ...
    ctx->acd->key_type = MBEDTLS_AES_SEC_KEY;
...
int mbedtls_aes_setseckey_dec(mbedtls_aes_context *ctx,
                              mbedtls_aes_sec_key_t *key)
{
    return mbedtls_aes_setseckey_enc(ctx, key);
}
...
{
    return mbedtls_aes_crypt_ecb_ext(ctx,
                                     mode,
                                     CE_AES_BLOCK_SIZE,
                                     input,
                                     output);
...
{
    int ret = CE_SUCCESS;
    ce_algo_t alg = CE_ALG_AES_ECB_NOPAD;

    /* 0. Check input parameters */
    if ((NULL == ctx) || (ctx->magic != MBEDTLS_AES_MAGIC)) {
        ret = CE_ERROR_BAD_PARAMS;
        goto finish;
    }
...
{
    int ret = CE_SUCCESS;
    ce_algo_t alg = CE_ALG_AES_ECB_PKCS7;
...
{
    int ret = CE_SUCCESS;
    uint8_t local_iv[CE_AES_BLOCK_SIZE] = {0};
    ce_algo_t alg = CE_ALG_AES_CBC_NOPAD;
     ...
     *    When the \p length is less than \c CE_AES_BLOCK_SIZE .
     *    IV will not be cached.
     */
    if ((MBEDTLS_AES_DECRYPT == mode) &&
       (length >= CE_AES_BLOCK_SIZE)) {
        pal_memcpy(local_iv,
    ...
    if (length >= CE_AES_BLOCK_SIZE) {
        if(MBEDTLS_AES_DECRYPT == mode) {
            pal_memcpy(iv, local_iv, CE_AES_BLOCK_SIZE);
        } else {
            pal_memcpy(iv,
                       output + length - CE_AES_BLOCK_SIZE,
                       CE_AES_BLOCK_SIZE);
        }
    }
                                ...
                                unsigned char *output)
{
    int ret = CE_SUCCESS;
    uint8_t local_iv[CE_AES_BLOCK_SIZE] = {0};
    ce_algo_t alg = CE_ALG_AES_CBC_PKCS7;
    size_t olen = 0;
    ...
    /**
     * 4. Update IV before decrypting on decryption.
     *    When the \p inlen is less than \c CE_AES_BLOCK_SIZE .
     *    IV will not be cached.
     */
    if ((MBEDTLS_AES_DECRYPT == mode) &&
        (inlen >= CE_AES_BLOCK_SIZE)) {
        pal_memcpy(local_iv,
                   input + inlen - CE_AES_BLOCK_SIZE,
                   CE_AES_BLOCK_SIZE);
    }
     ...
     * > \p olen is always not less than \c CE_AES_BLOCK_SIZE .
     **/
    if (MBEDTLS_AES_ENCRYPT == mode) {
        pal_memcpy(iv,
                   output + olen - CE_AES_BLOCK_SIZE,
                   CE_AES_BLOCK_SIZE);
    }
    if ((MBEDTLS_AES_DECRYPT == mode) &&
        (inlen >= CE_AES_BLOCK_SIZE)) {
                          ...
                          unsigned char *output)
{
    ...
    size_t left = length;
    size_t kpos = 0;
    uint8_t in_buf[CE_AES_BLOCK_SIZE] = {0};
    uint8_t out_buf[CE_AES_BLOCK_SIZE] = {0};
    ce_algo_t alg = CE_ALG_AES_CTR;
    ...
    kpos = *nc_off;
    if (kpos > 0) {
        prolen = MIN(CE_AES_BLOCK_SIZE - kpos, left);
        BS_XOR(output, input, stream_block + kpos, prolen);
        left -= prolen;
        ofs += prolen;
        kpos = (kpos + prolen) % CE_AES_BLOCK_SIZE;
    }
    ...
    /* 4. Set key from soft key and IV. */
    ret = _aes_set_key(ctx);
    if (CE_SUCCESS != ret) {
        goto error;
    }
    ...
    prolen = ROUND_DOWN(left, CE_AES_BLOCK_SIZE);
    ret = ce_cipher_crypt(&ctx->acd->cipher,
                          false,
                          prolen,
                          input + ofs,
    ...
    if (left > 0) {
        ...
        /* 8. Cache kstream. */
        BS_XOR(stream_block,
               input + length - CE_AES_BLOCK_SIZE,
               output + length - CE_AES_BLOCK_SIZE,
               CE_AES_BLOCK_SIZE);
    }
    *nc_off = left;
