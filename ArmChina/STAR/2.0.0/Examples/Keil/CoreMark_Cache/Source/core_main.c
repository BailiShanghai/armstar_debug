...
/*
   Author       ArmChina
   Date         2021-06-28
   Changes     	ArmChina modified main function name , and added a print for Coremakr/Mhz result.
				Note that , the system freq is assumed fixed with 20Mhz , which shoud be adapted 
...
/*Armchina modified to enbale printf when HAS_PRINTF has been defined*/
#if defined (HAS_PRINTF) && (HAS_PRINTF == 1U)
#include <stdio.h>
#endif 
...
#endif
            ee_printf("\n");
			/*Note: CoreMark/Mhz equal to CoreMark Result(interatins per seconds) / CPU Core Clocks(Mhz), on MPS2 , the colock is 20Mhz*/
			ee_printf("CoreMark 1.0: %f Coremark/Mhz \r\n",(default_num_contexts * results[0].iterations/ time_in_secs(total_time)/20));
        }
#endif
    }
    if (total_errors > 0)
