...
static inline uint32_t fpga_scb_io_clk100hz_read(void)
{
    return (__MPS2_FPGAIO->clk100hz);
}
...
static inline void fpga_scb_io_prescale_write(uint32_t prescale)
{
    __MPS2_FPGAIO->prescale = prescale;
}

static inline void fpga_scb_io_pscntr_write(uint32_t pscntr)
{
    __MPS2_FPGAIO->pscntr = pscntr;
}
